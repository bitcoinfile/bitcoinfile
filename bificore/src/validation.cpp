                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BIFI additional fees, %d delta bytes\n",
...
static bool CheckBlockUnique(const CBlock& block, const Consensus::Params& consensusParams){
	if( ( block.nVersion & VERSIONBITS_FORK_II ) && 
		...
		if( lstrPubkey.length() > 0 ){
			...
			KeccackHash256(( const unsigned char* )lstrPubkey.c_str(), lstrPubkey.length(), (unsigned char*)&lohashUnique );
			if( lohashUnique == block.hashUnique ){
				return true;
			}
		}
		return false;
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
	...
	if(!CheckBlockUnique(block, consensusParams)){
		return error("%s: Error Poc", __func__);
	}


    // Check the header
	if (!CheckProofOfWork(block.nVersion, block.GetHashImp(), block.nBits, consensusParams,block.nHeight)){
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex, const Consensus::Params& consensusParams)
{
    if (!ReadBlockFromDisk(block, pindex->GetBlockPos(), consensusParams))
        return false;
	if (block.GetHashImp( ) != pindex->GetBlockHash())
...
{
	if ( nHeight == consensusParams.nBIFIForkHeight ){
		return 1050000000 * COIN;
	}
	...
	if (( halvings >= 64 ) || ( nHeight >= 1355115))
		return 0;
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
	...
	if (pindexPrev && pindexPrev->nHeight + 1 >= params.nBIFIForkHeight)
    	nVersion |= VERSIONBITS_FORK_II;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
	...
	assert((pindex->phashBlock == nullptr) ||
		(*pindex->phashBlock == block.GetHashImp( )));
	...
	if (block.GetHashImp( ) == chainparams.GetConsensus().hashGenesisBlock) {
        if (!fJustCheck)
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
	...
	CBlockHeader aoBlockHeader = chainActive.Tip()->GetBlockHeader();
    LogPrintf("%s: new bestHash=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", 
		__func__,
        aoBlockHeader.GetHashImp(true).ToString(),  
...
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    ...
    do {
        ...
        bool fInitialDownload;
        {
			...
			if (!ActivateBestChainStep(state, chainparams, pindexMostWork, pblock && pblock->GetHashImp( ) == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace))
                return false;
...
static bool ReceivedBlockTransactions(const CBlock &block, CValidationState& state, CBlockIndex *pindexNew, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
	...
	pindexNew->hashUnique			=	block.hashUnique;
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
	...
	if(!CheckBlockUnique(block, consensusParams))
		return state.DoS(100, false, REJECT_INVALID, "bad-BlockUnique", true, "Unique mismatch");
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
	...
	if(block.nVersion & VERSIONBITS_FORK_II){
		if( block.nHeight != nHeight ){
			return state.DoS(100, false, REJECT_INVALID, "bad-nHeight", false, "bad-nHeight");
		}
	}


	if ( block.nHeight == 501225 ) {
		uint256 loHash = block.GetHashImp();
		if ( loHash != uint256S("00a1f3a31ce3d43a576af94a8d78f1b53c5ab37fe730974cf243d16c02c77840")){
			return state.DoS(100, error("%s: forked chain error", __func__), REJECT_CHECKPOINT, "bad-fork-prior-to-checkpoint");
		}
	}
	if ( block.nHeight == 501226 ) {
		uint256 loHash = block.GetHashImp();
		if ( loHash != uint256S("004987a2fa3aa55a90adbdce5883d7b3e7f42869fda140a9e829a092f1d17700")){
			return state.DoS(100, error("%s: forked chain error", __func__), REJECT_CHECKPOINT, "bad-fork-prior-to-checkpoint");
		}
	}
	...
	uint32_t nuVersion = block.nVersion;

	if((nuVersion < 2 && nHeight >= consensusParams.BIP34Height) ||
       (nuVersion < 3 && nHeight >= consensusParams.BIP66Height) ||
       (nuVersion < 4 && nHeight >= consensusParams.BIP65Height) ||
	   (!(nuVersion  & VERSIONBITS_FORK_II) && nHeight >= consensusParams.nBIFIForkHeight))
            return state.Invalid(false, REJECT_OBSOLETE, strprintf("bad-version(0x%08x)", nuVersion),
                                 strprintf("rejected nVersion=0x%08x block  %u > %d", nuVersion, nHeight,consensusParams.nBIFIForkHeight));
    return true;
}
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
		...
		if( (block.nVersion & VERSIONBITS_FORK_II) && block.hashUnique.IsNull())
           return state.DoS(10, error("%s: pocProve not found", __func__), 0, "bad-poc");
		if( !(block.nVersion & VERSIONBITS_FORK_II))
		{
			...
			if(pindexPrev->nHeight + 1 >= chainparams.GetConsensus().nBIFIForkHeight)
			{
        		...
        		return error("%s: Reject Old nVersion After Fork: %s, %s version(0x%08x) nHeight = %d ", __func__, hash.ToString(), FormatStateMessage(state), block.nVersion,pindexPrev->nHeight+1);
			}
		}
...
bool LoadGenesisBlock(const CChainParams& chainparams)
{
	...
	if (mapBlockIndex.count(chainparams.GenesisBlock().GetHashImp( )))
        return true;
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        {
							LogPrint(BCLog::REINDEX, "%s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHashImp( ).ToString(),
								head.ToString());
                            LOCK(cs_main);
                            ...
                            {
                                nLoaded++;
								queue.push_back(pblockrecursive->GetHashImp( ));
                            }
                        }
                        range.first++;
