...
double GetDifficulty(const CBlockIndex* blockindex)
{
	...
	int liMove = 29;
	if ( blockindex->nHeight > 501226 )
	{
	    liMove = 31;
	}else if (blockindex->nHeight==501226||blockindex->nHeight==501225 )
	{
		liMove = 32;
	}
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
	...
	uint32_t luVer = blockindex->nVersion;
...
UniValue getchaintxstats(const JSONRPCRequest& request)
{
    ...
    int blockcount = 30 * 24 * 60 * 60 / Params().GetConsensus().nBifiPowTargetSpacing; // By default: 1 month
